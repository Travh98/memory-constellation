extends Node
class_name BaseFrameTwoHandScaler


## Scales a frame's display contents when grabbed with two hands.
## Grab points spread with scale up to max_grab_width.
## Override _get_display_node() in subclasses to specify which node to scale.
...
func _process(_delta: float) -> void:
	...
	var current_distance: float = _get_distance(_hands[0], _hands[1])
...
func apply_scale() -> void:
	...
	var display: Node3D = _get_display_node()
	if display != null:
		display.scale = scale_vector
