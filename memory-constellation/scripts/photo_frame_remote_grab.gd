extends Node
class_name PhotoFrameRemoteGrab


## Handles remote (distance) grab for a PhotoFrame.
...
@onready var _scaler: PhotoFrameTwoHandScaler = $"../PhotoFrameTwoHandScaler"
...
func _build_manipulator(pos_a: Vector3, pos_b: Vector3, ref_up: Vector3 = Vector3.UP) -> Transform3D:
	var origin: Vector3 = (pos_a + pos_b) * 0.5 # Origin is middle point
